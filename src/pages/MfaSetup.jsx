...
import { ShieldCheck, Smartphone, Mail, Key, CheckCircle, Copy } from 'lucide-react';
...
export default function MfaSetup() {
  ...
  const verify = () => {
    if (code.length >= 6) { setEnabled(true); setShowVerify(false); setCode(''); setToast('MFA enabled successfully!'); setTimeout(() => setToast(''), 2500); }
  };

  return (
    <div>
      <PageHead title="Multi-Factor Authentication" sub="Add an extra layer of security to your account" />
      <div className="mt-5 grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
                ...
                <div className="flex flex-col sm:flex-row gap-5 items-start">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 bg-white px-4 py-2.5 rounded-lg border border-slate-200 mb-3">
                      ...
                      <button onClick={() => { navigator.clipboard?.writeText('JBSWY3DPEHPK3PXP'); setToast('Copied!'); setTimeout(() => setToast(''), 1500); }} className="p-1 text-slate-400 hover:text-teal-500"><Copy size={14} /></button>
                    </div>
          ...
          {/* Backup codes */}
          {enabled && <div className="bg-white rounded-2xl p-6 shadow-md border border-slate-100">
            <h3 className="text-base font-bold text-slate-900 font-display mb-1">Backup Codes</h3>
            <p className="text-sm text-slate-500 font-medium mb-4">Save these codes in a secure location. Each code can only be used once.</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {backupCodes.map((c, i) => (
                <div key={i} className="px-4 py-3 bg-slate-50 rounded-lg border border-slate-200 text-center">
                  <code className="text-sm font-mono font-bold text-slate-800">{c}</code>
                </div>
              ))}
            </div>
          </div>}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-amber-50 rounded-xl p-5 border border-amber-200">
            ...
            <p className="text-xs text-amber-700 font-medium leading-relaxed">If you lose access to your authenticator, you'll need backup codes to sign in. Store them securely.</p>
          </div>
        </div>
      </div>
